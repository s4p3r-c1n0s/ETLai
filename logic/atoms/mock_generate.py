...
def _generate_mock(headers: list[str], output_path: str, rows: int = 20) -> str:
...
def execute(params_json: str) -> str:
    """Generate mock CSVs from input file headers.

    Params:
        input_files: list of source CSV paths (reads headers only)
        target_path: directory to write mock outputs
        rows: number of rows to generate (default 20)
    """
    ...
    for file_path in input_files:
        ...
        _generate_mock(headers, output_path, rows)
        generated.append(output_path)

    return json.dumps({
        "success": True,
        "message": f"Generated {len(generated)} mock file(s) with {rows} rows each.",
