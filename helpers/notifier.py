"""Cross-platform native OS toast notifications. Supports macOS and Windows.
Clicking the notification opens a specified folder."""
...
def notify(title: str, message: str, open_folder: str | None = None, timeout: int = 5) -> None:
    """Send a native OS toast notification. Optionally opens a folder on click.
...
def _notify_windows(title: str, message: str, open_folder: str | None, timeout: int) -> None:
    ...
    [Windows.UI.Notifications.ToastNotificationManager]::CreateToastNotifier("Dagster Pipeline").Show($toast){launch_action}
    """
    subprocess.run(
